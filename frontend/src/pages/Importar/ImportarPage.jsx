...
const CAMPOS_CRM = [
  ...
  { key: 'telefono',        label: 'Teléfono Celular',    req: true,  desc: 'Celular principal (10 dígitos)' },
  { key: 'telefono2',       label: 'Teléfono Fijo/Alt.',  req: false, desc: 'Segundo teléfono' },
  { key: 'ciudad',          label: 'Ciudad',              req: false, desc: 'Ciudad del cliente' },
  { key: 'genero',          label: 'Género',              req: false, desc: 'MASCULINO / FEMENINO' },
  { key: 'direccion',       label: 'Dirección',           req: false, desc: 'Dirección completa' },
  { key: 'cedula',          label: 'Cédula / Documento',  req: false, desc: 'Número de identidad' },
]
...
export default function ImportarPage() {
  ...
  const [fuentes,      setFuentes]      = useState([])   // usaremos admin para obtener fuentes
  ...
  // ── Paso 3 → 4: Ejecutar importación ──────────────────────
  async function handleImportar() {
    if (!config.sala_id || !config.fuente_id || !config.tipificacion_id) {
      setError('Completa sala, fuente y tipificación antes de importar')
      return
    }
    setImportando(true)
    setError('')
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-5">
          ...
          >
            ...
            {archivo ? (
              ...
              <>
                <p className="text-gray-600 text-sm font-medium">Arrastra tu archivo aquí o haz clic para seleccionar</p>
                <p className="text-gray-400 text-xs mt-1">Solo archivos .xlsx o .xls (máx. 20 MB)</p>
              </>
            )}
          </div>
          <input
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800 space-y-1">
            ...
            <ul className="list-disc list-inside space-y-0.5 text-xs text-blue-700">
              <li>Solo nombre + teléfono (mínimo)</li>
              <li>Nombre + celular + teléfono fijo</li>
              <li>Nombre + celular + fijo + ciudad + género + dirección</li>
              <li>Los teléfonos pueden estar como 999xxxxxx o 0999xxxxxx o 00593...</li>
      ...
      {paso === PASO.MAPEO && preview && (
        <div className="space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {archivo.name} · Hoja: {preview.sheetName} · {preview.totalFilas} filas · {preview.totalCols} columnas
                </p>
              </div>
          ...
          {/* Mapeo de campos */}
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <h3 className="font-semibold text-gray-700 mb-4">¿Qué contiene cada columna?</h3>
      ...
      {paso === PASO.CONFIG && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-5">
          <h2 className="font-semibold text-gray-700">3. Configurar importación</h2>
          <p className="text-sm text-gray-500">
            Se importarán aprox. <strong>{preview.totalFilas - (tieneEncabezado ? 1 : 0)}</strong> registros del archivo <strong>{archivo.name}</strong>
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
              >
                <option value="">🔄 Distribuir automáticamente (round-robin)</option>
                {tmksDeSala.map(u => <option key={u.id} value={u.id}>{u.nombre}</option>)}
              </select>
              <p className="text-xs text-gray-400 mt-1">
                {config.tmk_id
                  ? 'Todos los leads se asignarán al TMK seleccionado'
                  : `Se distribuirán entre ${tmksDeSala.length || 0} TMKs activos de la sala`}
              </p>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Tipificación inicial *</label>
              <select
              ...
              {CAMPOS_CRM.map(campo => (
                mapeo[campo.key] !== null ? (
                  <span key={campo.key} className="text-xs bg-teal-100 text-teal-700 px-2 py-1 rounded-full">
                    {campo.label} → Col {letraColumna(mapeo[campo.key])}
                  </span>
                ) : campo.req ? (
          ...
          <div className="flex justify-between pt-2">
            ...
            <button
              ...
              disabled={importando || !config.sala_id || !config.fuente_id || !config.tipificacion_id}
      ...
      {paso === PASO.RESULTADO && resultado && (
        <div className="space-y-5">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <h2 className="font-semibold text-gray-700 mb-5">4. Resultado de la importación</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="rounded-xl p-4 bg-yellow-50 border border-yellow-200 text-center">
                <p className="text-xs font-semibold text-yellow-600 uppercase">Duplicados</p>
                <p className="text-3xl font-bold text-yellow-700 mt-1">{resultado.duplicados}</p>
                <p className="text-xs text-yellow-500 mt-0.5">ya existían en la sala</p>
              </div>
            ...
            {resultado.importados > 0 && (
              <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl">
                <p className="text-green-800 text-sm font-semibold">
                  ✅ Se crearon {resultado.importados} leads nuevos y se asignaron a los TMKs de la sala.
                </p>
